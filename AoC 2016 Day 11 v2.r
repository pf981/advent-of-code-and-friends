...
a = FIXME_REMOVE_possible_items[[1]]
a[[2]]
...
#   !all(floors_with_exposed_chip %in% floors_with_generator)
# }
...
#     FIXME_REMOVE_states <<- c(FIXME_REMOVE_states, list(state))


#     # All possible moves
#     possible_items <- c(
#       NA,
#       which(start_state == elevator, arr.ind = TRUE) %>% asplit(1)
#     )
#     FIXME_REMOVE_possible_items <<- c(FIXME_REMOVE_possible_items, list(possible_items))
#     for (new_elevator in c(elevator + 1, elevator - 1)) {
#       if (new_elevator < 1 || new_elevator > 4) {
#         next
#       }
      
#       for (item1 in seq_along(possible_items)) {
#         for (item2 in seq_along(possible_items)[seq_along(possible_items) > item1]) {
#           new_state <- state
#           for (item in c(item1, item2)) {
#             if (!is.na(possible_items[[item]])) {
#               new_state[possible_items[[item]][["row"]], possible_items[[item]][["col"]]] <- new_elevator
#             }
#           }
          
#           state_hash <- hash(new_state)
#           FIXME_REMOVE_hashes <<- c(FIXME_REMOVE_hashes, state_hash)
#           FIXME_REMOVE_new_state <<- c(FIXME_REMOVE_new_state, list(new_state))
#           if (state_hash %in% done_states) {
#             next
#           }
          
#           if (!is_valid(state)) {
#             FIXME_REMOVE_non_valid_states <<- c(FIXME_REMOVE_non_valid_states, list(state))
#             next
#           }
...
# # FIXME: REMOVE
# FIXME_REMOVE_states <- list()
# FIXME_REMOVE_non_valid_states <- list()
# FIXME_REMOVE_hashes <- c()
# FIXME_REMOVE_new_state <- list()
# FIXME_REMOVE_possible_items <- list()
# solve()

# COMMAND ----------

# FIXME_REMOVE_possible_items[[1]][[2]][["row"]]
FIXME_REMOVE_possible_items[[1]][[2]][["col"]]

# COMMAND ----------

FIXME_REMOVE_possible_items %>% str()
...
#         for (item2 in seq_along(possible_items)[seq_along(possible_items) > item1]) {
#           if (is.na(item1)) {
#             items_rows <- possible_items[item2][["row"]]
#             items_cols <- possible_items[item2][["col"]]
#           } else {
#             items_rows <- c(possible_items[item1][["row"]], possible_items[item2][["row"]])
#             items_cols <- c(possible_items[item1][["col"]], possible_items[item2][["col"]])
#           }
            
#           new_state <- state
#           new_state[items_rows, items_cols] <- new_elevator
