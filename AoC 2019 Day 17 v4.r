...
# `[.instructions` <- function(v, i) {
#   message(glue::glue("{i[1]} {i[2]}"))
#   index <- to_character(get_index(v, i))
#   message(glue::glue("INDEX: {index}"))
#   tryCatch(result <- v[[index]], error = function(cond) v[[index]] <<- 0)
#   if (is.null(result) || is.na(result)) v[[index]] <- 0
#   message(glue::glue("END"))
#   tryCatch(v[[index]], error = function(cond) v[[index]] <<- 0)
#   v[[index]]
# }
...
# bot$instructions[['hello']]

# COMMAND ----------

# bot$instructions[list(96000, 2)]

# COMMAND ----------

# bot$instructions[list(960, 1)]
