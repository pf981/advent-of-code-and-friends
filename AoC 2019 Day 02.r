...
# MAGIC %md > before running the program, replace position 1 with the value 12 and replace position 2 with the value 2. What value is left at position 0 after the program halts?
...
run_instructions_modified <- function(instructions, pos_1, pos_2) {
...
find_solution <- function() {
  for (pos_1 in seq(from = 0, to = 99)) {
    for (pos_2 in seq(from = 0, to = 99)) {
      instructions <- instructions_original
      if (run_instructions_modified(instructions, pos_1, pos_2)[[1]] == 19690720) {
