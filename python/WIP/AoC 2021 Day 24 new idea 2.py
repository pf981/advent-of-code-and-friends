...
zs = [{} for _ in range(15)] # step: output_z, digits
zs[14][0] = (0, {''})

# COMMAND ----------

def get_input(step, output_z):
  return (input_z, digits)

for step in range(13, -1, -1):
  digits = [get_input(step, output_z) for output_z in zs[step + 1]]
...
zs = collections.defaultdict(set, {0: 0})
z, digit
...
zs = [{} for _ in range(15)] # step: z, digit
zs[14][0] = (0, '')
...
def solve(i, z):
  ...
  for d in range(9, 0, -1):
    ...
    other_digits = solve(i + 1, z)
    if other_digits is not None:
      return str(d) + other_digits
