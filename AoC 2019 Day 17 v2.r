...
while(!bot$is_halted) bot <- run_bot(bot, 10)
...
# NOTE: Your next attempt you should just enable the video feed and see what is happening. Maybe you have off by 1? I don't think my lengths are off by one, though because it matches the example

# COMMAND ----------

# What happens if I just keep feeding in newlines when it asks for more input?
# If this infinite loops, maybe check halted
bot2 <- bot
while(bot2$output < 256) bot2 <- run_bot(bot2, 10)
answer <- bot2$output
answer
...
# Try flipped in case my orientation was wrong
input_sequence_str <- "A,B,A,B,C,B,C,A,C,C
R,12,L,10,L,10
L,6,L,12,R,12,L,4
L,12,R,12,L,6
...
"
input_sequence <- utf8ToInt(input_sequence_str)
input_sequence <- case_when(
  input_sequence == utf8ToInt('R') ~ utf8ToInt('L'),
  input_sequence == utf8ToInt('L') ~ utf8ToInt('R'),
  TRUE ~ input_sequence
)
...
# instructions[1] <- 2 # I think this isn't working

bot <- create_bot(sequence)
bot$instructions[list(0, 1)] <- 2
bot <- run_bot(bot, input_sequence)
...
# I wonder if my graph is rotated? So that l is r?
...
get_dust_collected <- function(instructions, input) {
  bot <- create_bot(instructions)
  while (!bot$is_halted) {
    bot <- run_bot(bot, input)
  }
  bot$output
}
...
answer <- get_dust_collected(new_sequence, input = input_sequence)
answer
...
# Need to enumerate all the possible paths. I.e. take all possible turns at intersections

# Actually, I can just see what path it is meant to take - it isn't going to do crazy short zig-zags.
...
# result <- get_output(new_sequence)
# result
...
target_input <- "R,12,L,10,L,10,L,6,L,12,R,12,L,4,R,12,L,10,L,10,L,6,L,12,R,12,L,4,L,12,R,12,L,6,L,6,L,12,R,12,L,4,L,12,R,12,L,6,R,12,L,10,L,10,L,12,R,12,L,6,L,12,R,12,L,6" %>%
  str_split(",") %>%
  unlist()
target_input
...
check_if_valid <- function(a_n, b_n, c_n, target_input) {
  check_str <- str_c(target_input, collapse = "")
  
  a_str <- head(target_input, a_n) %>% str_c(collapse = "")
  target_input <- tail(target_input, -a_n)
  
  b_str <- head(target_input, b_n) %>% str_c(collapse = "")
  target_input <- tail(target_input, -b_n)
  
  c_str <- head(target_input, c_n) %>% str_c(collapse = "")
  ...
  for (o in orders) {
    ...
    if (any(o == "")) next
    for (to_remove in o) {
      str_remove_all(check_str, to_remove)
    }
...
possible_function_lengths <- seq_len(length(target_input))
possible_df <-
  crossing(
    a_n = possible_function_lengths,
    b_n = possible_function_lengths,
    c_n = possible_function_lengths
  ) %>%
  mutate(
    is_valid = pmap_lgl(list(a_n, b_n, c_n), check_if_valid, target_input = target_input)
  )
...
# STEP 0: R,12,L,10,L,10,L,6,L,12,
# STEP 1: R,12,L,4,R,12,L,10,L,10,L,6,L,12
# Step 2: R,12,L,4,L,12,R,12,L,6,L,6,L,12,R,12,L,4
# Step 3: L,12,R,12,L,6,R,12,L,10,L,10
# Step 4: L,12,R,12,L,6,L,12,R,12,L,6
