...
def count_gears(r: int, c: int, seen: set[tuple[int, int]]) -> int:
    if (r, c) in seen:
        return 0
    seen.add((r, c))
    for dr, dc in [(-1, 0), (0, -1), (0, 1), (1, 0)]:
        r2 = r + dr
        c2 = c + dc
        if (r2, c2) in seen or (r2, c2) not in gears:
            continue
        count_gears(r2, c2, seen)
    return len(seen)
...
while stack:
    ...
    for dr, dc in [(-1, 0), (0, -1), (0, 1), (1, 0)]:
        ...
        if (r2, c2) in b_in and not is_prime(
            count_gears(*b_out[b_in[(r2, c2)]], set()) - 1
        ):
