...
def recalc(node: Treap | None) -> None:
    ...
    if node.lazy_reverse:
        ...
        node.is_flipped = not node.is_flipped
...
def split_off_max(node: Treap, max_val: int) -> tuple[Treap | None, Treap | None, bool]:
    recalc(node)

    if node.val == max_val:
        return (node.left, node.right, node.is_flipped)

    # max_value is in left subtree, this node is in the right split
    if node.left and node.left.largest == max_val:
        l, r, is_flipped = split_off_max(node.left, max_val)
        node.left = None
        r = merge(r, node)
        return (l, r, is_flipped)

    # max_value is in right subtree, this node is in the left split
    if node.right and node.right.largest == max_val:
        l, r, is_flipped = split_off_max(node.right, max_val)
        node.right = None
        l = merge(node, l)
        return (l, r, is_flipped)
...
for num in re.findall(r"-?\d+", text):
    ...
    treap = merge(
        treap,
        Treap(priority=random.random(), val=num, largest=num, is_flipped=is_flipped),
    )

flips = 0
while treap:
    l, r, is_flipped = split_off_max(treap, treap.largest)

    # If right split is empty, largest was in the correct position
    if not r and not is_flipped:
        treap = l
        continue
