...
def test_solution23(r):
    ...
    for _ in range(100):
        dx, dy, rhs = random.choice(triples)

        ax = random.randint(1, 30)
        ay = random.randint(1, 30)

        bx = ax + dx
        by = ay + dy

        ax, bx = random.sample([ax, bx], k=2)
        ay, by = random.sample([ay, by], k=2)

        assert r(f"{tally(ax)},{tally(ay)};{tally(bx)},{tally(by)}") == tally(rhs)
