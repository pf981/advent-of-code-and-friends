...
def test_postagestamp_4():
    def sat(
        ...
        target=56,
        max_stamps=1,
        options=[25, 22, 8, 84, 60, 56, 54, 7, 8],
    ):
        for s in stamps:
            assert s in options
        return len(stamps) <= max_stamps and sum(stamps) == target
...
def test_necklacesplit_3():
    def sat(
        n: int,
        lace="bbbbrrbbbbrrbbrrrbbrrbbrrrrrrrbrbrbbbrrbrrrbbbbbbbrbrbrbbbbbbbrrbbrbbrbrrbrbrrbbbrrrrrbrrbbrrrbbrbrrrbbbbrbbbrrrrbrbrrbbrbrbrbbrrbrrrbrbrrbbbbbbrbrrrrbbrbbbrbrrbrbbrbrrbbbbrrrrrbrrrbbrrrrrrbrrrbrbbbrbbbrrrbbr",
    ):
        sub = lace[n : n + len(lace) // 2]
        return (
            n >= 0
...
def test_cardgame24():
    def sat(expr: str, nums=[3, 7, 3, 7]):
        ...
        for i in range(len(expr)):
            ...
            assert expr[i] in "1234567890()+-*/", (
                "Expr can only contain `0123456789()+-*/`"
            )
            digits += expr[i] if expr[i] in "0123456789" else " "
        assert sorted(int(s) for s in digits.split()) == sorted(nums), (
