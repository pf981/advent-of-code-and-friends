...
output = Computer(instructions).run().output_values

answer = sum(tile_id == 2 for i, tile_id in enumerate(output) if i % 3 == 2)
print(answer)
...
while computer.run().state != State.HALTED:
  chunked = [computer.output_values[i:i + 3] for i in range(0, len(computer.output_values), 3)]
  ball_x = next(x for x, _, tile_id in chunked if tile_id == 4)
  paddle_x = next((x for x, _, tile_id in chunked if tile_id == 3), paddle_x)

  computer.output_values.clear()
  computer.input_values.append((ball_x > paddle_x) - (ball_x < paddle_x))

answer = computer.output_values[-1]
print(answer)
